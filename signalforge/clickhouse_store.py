...
ReplacingMergeTree keyed on (day, entity_id) turns the pipeline's upserts into plain inserts;
...
TABLE = "signals_daily"
COLUMNS = ["day", "entity_id", "window_start", "window_end", "n", "mean_score", "min_score",
...
GET_SQL = SELECT + "day = {day:Date} AND entity_id = {entity_id:String}"
MGET_SQL = SELECT + "day IN {days:Array(Date)} AND entity_id = {entity_id:String}"
...
def doc_to_row(doc: Dict, updated_at: Optional[dt.datetime] = None) -> List:
    ...
    return [dt.date.fromisoformat(doc["day"]), doc["entity_id"],
...
class ClickHouseStore:
    ...
    def get(self, index: str, doc_id: str) -> Optional[Dict]:
        docs = self._select(GET_SQL, {"day": dt.date.fromisoformat(day_of(index)), "entity_id": doc_id})
        return docs[0] if docs else None

    def mget(self, indices: List[str], doc_id: str) -> List[Dict]:
        days = [dt.date.fromisoformat(day_of(i)) for i in indices]
        return self._select(MGET_SQL, {"days": days, "entity_id": doc_id})
...
class InMemoryClickHouseStore:
    ...
    def __init__(self) -> None:
        self.rows: List[Tuple[str, str, float, Dict]] = []  # (day, entity_id, version, doc)
        self._ver = 0.0
    ...
    def bulk_upsert(self, index: str, docs: Iterable[Dict]) -> int:
        ...
        n = 0
        for d in docs:
            self.rows.append((day, d["entity_id"], self._ver, dict(d)))
            n += 1
        return n
