...
def test_pipeline_writes_the_rollups_to_delta_and_a_day_can_be_replaced_in_place(
    ...
    assert run_batch(spark, archive, store, cfg, lake=lake) == 4
    table = lake.read(spark)
    ...
    assert [f.name for f in spark.read.format("delta").load(lake.path).schema][-1] == "day"

    # replaying one day rewrites just that partition: still 4 rows, and one more commit
    assert run_batch(spark, archive, store, cfg, day=D2, lake=lake) == 2
    assert lake.read(spark).count() == 4
    assert lake.read(spark, day=D1).count() == 2
    versions = [c.version for c in lake.history(spark)]
    assert versions == sorted(versions, reverse=True) and len(versions) == 2
    assert lake.latest(spark).version == 1


def test_schema_evolution_and_time_travel(spark, tmp_path):
    ...
    old = spark.createDataFrame([(D1, "t-0", "ent-1", 3.5, 10)],
                                      "day string, tenant_id string, entity_id string, "
                                      "mean_score double, n long")
    new = spark.createDataFrame([(D2, "t-0", "ent-1", 4.0, 12, 0.25)],
                                      "day string, tenant_id string, entity_id string, "
    ...
    v0 = lake.read(spark, version=0)
    assert "stddev_score" not in v0.columns and v0.count() == 1

    commits = lake.history(spark)
    ...
    assert lake.read(spark, as_of=as_of).count() == 1
